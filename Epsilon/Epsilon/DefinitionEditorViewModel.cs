...
namespace Epsilon
{
	public class DefinitionEditorViewModel : TagEditorPlugin, ITagEditorPlugin
	{
		...
		public Epsilon.Commands.DelegateCommand ExpandAllCommand { get; }

		public Epsilon.Commands.DelegateCommand CollapseAllCommand { get; }

		public Epsilon.Commands.DelegateCommand SaveCommand { get; }

		public Epsilon.Commands.DelegateCommand PokeCommand { get; }

		public Epsilon.Commands.DelegateCommand ReloadCommand { get; }
		...
		public DefinitionEditorViewModel(TagEditorContext context, params object[] args) : base(context) {
			...
			ExpandAllCommand = new Epsilon.Commands.DelegateCommand(ExpandAll);
			CollapseAllCommand = new Epsilon.Commands.DelegateCommand(CollapseAll);

			PokeCommand = new Epsilon.Commands.DelegateCommand(PokeChanges, () => RteTargetList.Any());
			SaveCommand = new Epsilon.Commands.DelegateCommand(SaveChanges, () => _cacheFile.CanSerializeTags);
			ReloadCommand = new Epsilon.Commands.DelegateCommand(_cacheEditor.ReloadCurrentTag);
		...
		internal bool PopulateContextMenu(Node menu, IField field) {
			...
			if (field is ValueField vf) {
				...
				if (field is ValueField value && value.IsNot<BlockField, DataField, InlineStructField>()) {
					...
					_ = menu.Submenu(Menus.Item.k_FieldEditor)

						.Add("Poke Field", new Epsilon.Commands.DelegateCommand(delegate { PokeField(value); }),
				...
				if (field3 is StringIdField stringIdField) {
					...
					_ = menu.Submenu(Menus.Item.k_FieldEditor)
						...
						.Add("Edit Unicode String", new Epsilon.Commands.DelegateCommand(delegate { EditUnicString(stringIdField); }),
				...
				if (!blockOrStruct) {
					...
					_ = menu.Submenu(Menus.Item.k_FieldEditor)
							...
							.Add("Copy SetField Command", new Epsilon.Commands.DelegateCommand(delegate { CopySetFieldCommand(vf); }),
				...
				_ = menu.Submenu(Menus.Item.k_FieldEditor)
						...
						.Add("Copy Name", new Epsilon.Commands.DelegateCommand(delegate { CopyFieldName(vf); }),
							 "Copies the name of this field")
						
						.Add("Copy Path", new Epsilon.Commands.DelegateCommand(delegate { CopyFieldPath(vf); }),
							 "Copies the path of this field");

				if (!blockOrStruct) {

					_ = menu
						.Submenu(Menus.Item.k_FieldEditor)

							.Add("Copy Value", new Epsilon.Commands.DelegateCommand(delegate { CopyFieldValue(vf); }),
								 "Copies the value of this field")

							.Add("Copy Path + Value", new Epsilon.Commands.DelegateCommand(delegate { CopyFieldPathWithValue(vf); }),
				...
				_ = menu.Submenu(Menus.Item.k_FieldEditor)
					...
					.Add("Copy Offset", new Epsilon.Commands.DelegateCommand(delegate { CopyFieldOffset(vf); }),
			...
			_ = menu.Submenu(Menus.Item.k_FieldEditor)
				...
				.Add("Copy Memory Address", new Epsilon.Commands.DelegateCommand(delegate { CopyFieldMemoryAddress(field); }, RteHasValidTargets), 
